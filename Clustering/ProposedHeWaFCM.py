...
class ProposedHeWaFCM:
    ...
    def Hemingway_distance (self, ts_data, val_data,cluster_centers):
        def sd_calc(data):
            n = len(data)
            if n <= 1:
                return 0.0
        ...
        def avg_calc(ls):
            ...
            return mean
        ans = 0
        for i in range(ts_data):
            math.sum(math.sd_calc(ts_data,val_data))
            if ts_data==cluster_centers:
                break
